...
39/54 studies \\
RQ2 & Efficiency &
Token-level routing achieves $>$80\% cost reduction; orchestration overhead polarizes latency; energy is the least-reported metric across the corpus. &
47/54 studies \\
RQ3 & Orchestration &
...
The 54 included studies cluster into four recurring architectural patterns. Table~\ref{tab:concept-matrix} maps each pattern to its dominant orchestration mechanisms, the engineering constraints it addresses, and the primary studies assigned to it. This concept matrix provides the organizing structure for the research-question-based analysis that follows.
...
\textbf{Key Engineering Constraints} &
...
S2, S13, S27, S32, S33, S45, S46, S48, S49, S53. \\
...
S1, S6, S7, S11, S15, S16, S20, S29, S37, S43, S51, S52, S54. \\
...
Hardware limits on CPUs and edge devices, and non-text modality processing. &
S3, S5, S8, S9, S12, S14, S17--S19, S22--S26, S28, S34--S36, S38--S42, S47, S50. \\
\bottomrule
\end{tabularx}
\end{table}
...
\addplot[fill=black!62, draw=black!80] coordinates {
    (2023,1)
    (2024,13)
    (2025,38)
    (2026,2)
...
\pie[
    ...
    14/{Healthcare \& medicine},
    8/{Software engineering \& code},
    6/{Telecommunications \& networks},
    6/{Edge, IoT, \& industrial},
    4/{Education \& assessment},
    4/{Cybersecurity \& science},
    12/{General NLP \& benchmarking}
}
\end{tikzpicture}
\caption{Applied-domain distribution of the 54 included studies (\(n\) per slice shown in legend).}
...
\begin{itemize}
    \item Hybrid systems are strongest when selective escalation balances model quality against latency and cost.
...
\begin{itemize}
    ...
    \item Energy claims remain weak without direct, hardware-specific measurements that account for generation length and orchestration cost.
\end{itemize}
\end{samepage}
...
\smallskip\noindent\textit{Key takeaways.}
\begin{itemize}
    \item Healthcare and privacy-sensitive domains favor local, retrieval-grounded models with verifiable evidence.
