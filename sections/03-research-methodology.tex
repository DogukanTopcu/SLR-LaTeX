...
\begin{table}[!htbp]
    ...
    \begin{tabularx}{\linewidth}{@{} p{2.8cm} >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X @{}}
        ...
        Studies centered on SLM-only systems or strictly comparing massive monolithic LLMs (e.g., GPT-4 vs. Gemini) without a collaborative SLM component; monolithic LLMs were considered only as baselines within otherwise eligible studies. \\
        \addlinespace % Adds a subtle gap between rows
        
        \textbf{Model Size and Transparency} & 
        Architectures with an identifiable SLM component whose parameter count is reported or inferable and does not exceed 7 billion parameters. Closed-box LLMs were allowed only when used as baselines, teachers, or fallback components inside an otherwise eligible collaborative architecture. & 
        Studies whose primary evaluated architecture depends on closed-box or API-bound models with undisclosed parameter counts or architectural details, or where no reproducible SLM component can be identified. \\
        \addlinespace
