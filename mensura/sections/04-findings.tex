...
Architecture-type assignment in Table~\ref{tab:concept-matrix} follows a primary-category, mutually exclusive coding scheme: each of the 54 studies is assigned to the single architectural pattern that best characterizes its \emph{dominant} orchestration mechanism, even when secondary traits of other patterns are also present (e.g., S14 is coded as a hybrid architecture in Table~\ref{tab:concept-matrix} but is also discussed as a task-decomposition design in Section~\ref{sec:rq3}). Counts in the table therefore sum to exactly 54 by construction and should not be read as a multi-label tally.
...
\mbox{S3}, \mbox{S5}, \mbox{S10}, \mbox{S13}, \mbox{S14}, \mbox{S15}, \mbox{S24}, \mbox{S40}, \mbox{S48}, \mbox{S49}. \\
...
\mbox{S6}, \mbox{S9}, \mbox{S16}, \mbox{S18}, \mbox{S21}, \mbox{S23}, \mbox{S26}, \mbox{S29}, \mbox{S37}, \mbox{S43}, \mbox{S52}, \mbox{S53}, \mbox{S54}. \\
...
\mbox{S11}, \mbox{S17}, \mbox{S30}, \mbox{S38}, \mbox{S39}, \mbox{S47}. \\

\textbf{Ensembles and Heterogeneous Fusion} &
Tabular transformation, cross-modal late fusion, and prompt-guided multimodal sampling; also the residual category for studies without a distinct collaborative mechanism. &
Hardware limits on central processing units (CPUs) and edge devices, and non-text modality processing. &
\mbox{S1}, \mbox{S2}, \mbox{S4}, \mbox{S7}, \mbox{S8}, \mbox{S12}, \mbox{S19}, \mbox{S20}, \mbox{S22}, \mbox{S25}, \mbox{S27}, \mbox{S28}, \mbox{S31}, \mbox{S32}, \mbox{S33}, \mbox{S34}, \mbox{S35}, \mbox{S36}, \mbox{S41}, \mbox{S42}, \mbox{S44}, \mbox{S45}, \mbox{S46}, \mbox{S50}, \mbox{S51}. \\
\bottomrule
\end{tabularx}
\end{table}
...
The review shows that both hybrid and multi-agent architectures can outperform standalone small-model baselines, but their advantages emerge through different forms of coordination rather than parameter scaling alone. Hybrid systems achieve their strongest gains when computation is selectively distributed between local small models and stronger remote models. Token-level edge--cloud collaboration improves small-model performance without requiring full-query escalation: draft-verification, low-confidence routing, and uncertainty-aware skipping allow small models to approach LLM-level quality while invoking the larger model selectively (S13, S48, S49). At the sub-task level, structured task decomposition and cloud--edge allocation also maintain reasoning accuracy comparable to the best baselines (S14). In domain-specific settings, hybrid orchestration allows a much smaller model to close the gap with larger general-purpose models: a fine-tuned Phi-2 (2.7B) combined with two-stage retrieval, cross-encoder reranking, and long-context support reaches 81.20\% accuracy on telecommunications question answering (QA)---a 30\% gain over the base Phi-2 and on par with substantially larger LLMs, at higher faithfulness (S17).
...
\begin{tikzpicture}[
  ...
  \node[res, right=2.0cm of d1] (r1) {\textbf{RAG architecture}\\[2pt]S17, S19, S23, S30};

  \node[dec, below=of d1] (d2) {Strict, interactive-style latency budget (e.g., real-time or voice, sub-second user-facing response)?};
  \node[res, right=2.0cm of d2] (r2) {\textbf{Hybrid token routing}\\[2pt]S37, S40, S48, S49};

  \node[dec, below=of d2] (d3) {Multi-step reasoning or complex, decomposable task?};
  \node[res, right=2.0cm of d3] (r3) {\textbf{Multi-agent decomp.}\\[2pt]S15, S29, S43, S52};

  \node[dec, below=of d3] (d4) {Non-text or multimodal input?};
  \node[res, right=2.0cm of d4] (r4) {\textbf{Ensemble / fusion}\\[2pt]S1, S8, S19, S20};
