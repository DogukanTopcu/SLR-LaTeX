...
To capture a comprehensive and high-quality corpus of contemporary collaborative language model architectures, the literature search was centered on the Web of Science (WoS) Core Collection, with the final search performed on 8 February 2026. Exploratory searches using the same core concepts were additionally run in Scopus and the Association for Computing Machinery (ACM) Digital Library to gauge coverage. These did not surface eligible studies outside the WoS result set, but they were an informal check rather than a formal screening stage: their records were neither deduplicated against the WoS set nor assessed individually against the eligibility criteria, and they consequently do not enter the selection flow reported in Figure~\ref{fig:prisma}. The corpus is therefore WoS-based, and the implications of this for coverage are discussed under external validity (Section~\ref{sec:threats}).
